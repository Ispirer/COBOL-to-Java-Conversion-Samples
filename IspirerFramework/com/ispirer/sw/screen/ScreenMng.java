...
*/
package com.ispirer.sw.screen;

import com.ispirer.sw.types.Form;
import com.ispirer.sw.types.Program;
...
public class ScreenMng {
	...
	private ScreenMng() {
		UIManager.put("TextField.caretForeground", new ColorUIResource(Color.green));
		form.setName("FVBX01");
		form.setBounds(268, 106, 900, 613);
		form.setTitle("");
	...
	public static void formvrto(Program program, Form form, Object data) {
		form.initData(data.toString());
		form.initActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				program.procedureDivision(form.output().toCharArray());
				getForm().dispose();
			}
		});
		form.initForm();
	...
	public static void clear() {
		getForm().getContentPane().removeAll();
		getForm().getContentPane().revalidate();
		getForm().getContentPane().repaint();
	}
